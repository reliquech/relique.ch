...
export function CustomerForm({
  ...
  useEffect(() => {
    ...
    setTagsInput(next);
    const tagsArray = next.split(",").map((t) => t.trim()).filter(Boolean);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <Input {...register("full_name")} className="bg-white/5 border-border text-white mt-1" placeholder="Full name" />
        {errors.full_name && <p className="text-destructive text-xs mt-1">{errors.full_name.message}</p>}
      </div>
      <div>
        <Label className="text-gray-300">Email</Label>
        <Input {...register("email")} type="email" className="bg-white/5 border-border text-white mt-1" placeholder="email@example.com" />
      ...
      <div>
        ...
        <Input {...register("address")} className="bg-white/5 border-border text-white mt-1" placeholder="Address" />
      </div>
      <div>
        <Label className="text-gray-300">Tags</Label>
        <Input value={tagsInput} onChange={(e) => handleTagsChange(e.target.value)} className="bg-white/5 border-border text-white mt-1" placeholder="vip, wholesale, nyc" />
