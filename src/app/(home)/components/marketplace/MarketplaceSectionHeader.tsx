...
export function MarketplaceSectionHeader({ itemsCount, className = "" }: MarketplaceSectionHeaderProps) {
  const showDragInstruction = itemsCount > 1;

  return (
    <div className={`w-full flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6 ${className}`}>
      <motion.div
